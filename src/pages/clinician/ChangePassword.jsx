...
import { ArrowLeft, Shield, Check, Eye, EyeOff, Loader2, Lock } from "lucide-react"
import { motion } from "framer-motion"
import { useAuth } from "@/hooks/useAuth"
import { changePassword } from "@/api"
import { toast } from "sonner"

export default function ClinicianChangePassword() {
  ...
  return (
    <div className="min-h-screen bg-[#F8FAFC] pb-20 relative overflow-hidden">
      {/* Decorative Watermark - Blue Theme */}
      <div className="absolute bottom-10 right-10 opacity-[0.03] pointer-events-none transform rotate-12">
        <Shield size={400} className="text-blue-900" />
        <div className="absolute inset-0 flex items-center justify-center">
             <Check size={200} className="text-blue-900" />
        </div>
      </div>
      ...
      <main className="max-w-xl mx-auto mt-10 px-6 relative z-10">
        <motion.div 
            ...
            className="space-y-8"
        >
          {/* Requirements Box - Blue Theme */}
          <div className="bg-blue-50 border border-blue-100 rounded-[2rem] p-8 shadow-sm">
            <div className="flex items-center gap-3 mb-6">
                <div className="p-2.5 bg-white border border-blue-100 text-blue-600 rounded-xl shadow-sm">
                    <Shield size={20} />
                </div>
                <div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-y-4 gap-x-8">
              {requirements.map((req, i) => (
                <div key={i} className="flex items-center gap-3 group whitespace-nowrap">
                   <div className={`p-1 rounded-full transition-all duration-300 ${req.met ? 'bg-blue-500 text-white scale-110' : 'bg-white text-gray-300 border border-gray-200'}`}>
          ...
          <form onSubmit={handleUpdate} className="space-y-6">
            ...
            <div className="space-y-2">
                ...
                <div className="relative group">
                    <input 
                        ...
                        className="w-full bg-gray-100 border-2 border-transparent focus:border-blue-500 focus:bg-white focus:ring-4 focus:ring-blue-500/10 rounded-2xl py-4 px-6 transition-all font-black text-gray-900 text-sm placeholder:text-gray-300"
                        placeholder="••••••••"
                    />
                    <button 
                        type="button"
            ...
            <button 
                type="submit"
                disabled={loading}
                className="w-full bg-blue-600 hover:bg-blue-700 text-white font-black py-5 rounded-[2rem] shadow-xl shadow-blue-500/20 transition-all hover:scale-[1.02] active:scale-[0.98] disabled:opacity-50 disabled:grayscale disabled:scale-100 flex items-center justify-center gap-3 mt-4"
            >
