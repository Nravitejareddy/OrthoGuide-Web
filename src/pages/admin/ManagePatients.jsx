import { Search, Loader2, Download, UserPlus, Shield, User, Key, Trash2, UserCheck } from "lucide-react"
...
export default function ManagePatients({ user }) {
  ...
  const patients = (Array.isArray(usersList) ? usersList : []).filter(u => 
  ...
  const handleExport = () => {
    try {
      ...
      const rows = patients.map(u => [u.id, u.name, u.email, u.is_active ? 'ACTIVE' : 'INACTIVE']);
  ...
  return (
    <div className="space-y-8 animate-fade-up max-w-7xl mx-auto pb-20">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-6">
        <div>
          <h1 className="text-3xl font-black text-gray-900 tracking-tight leading-none">Manage Patients</h1>
          <p className="text-gray-400 font-medium text-sm mt-2">{patients.length} Patients registered</p>
        </div>
        <div className="flex items-center gap-3 w-full sm:w-auto">
          ...
          <button
            onClick={() => navigate("/dashboard/admin/patients/new")}
      ...
      <div className="flex flex-col gap-3 px-2">
        {patients.map((u) => (
          <UserCard 
            key={u.id}
            user={u}
            onEdit={(user) => navigate(`/dashboard/admin/patients/${user.id}`)}
        ...
        {patients.length === 0 && (
          <div className="py-20 text-center bg-white border border-dashed border-gray-100 rounded-3xl">
            ...
            <p className="text-gray-400 font-bold tracking-tight">No patients found matching your search.</p>
          </div>
        )}
      </div>
